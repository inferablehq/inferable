      ...
      >
        ...
        <div className="flex-1 min-w-0 pt-0.5">
          <div className="flex items-baseline gap-2 flex-wrap mb-1">
            ...
            {node.label && (
              <code className="px-1.5 py-1 bg-muted rounded text-xs font-mono">
                {node.label}
              </code>
            )}
            {node.interactive && (
...
export default function WorkflowExecutionDetailsPage({
  ...
  const handleNodeClick = (node: Node) => {
    if (node.title === "Agent Run") {
      setSelectedRunId(selectedRunId ? null : node.id);
    } else {
      setSelectedEventId(selectedEventId ? null : node.id);
    }
  };
  ...
  return (
    <div className="p-6 flex gap-6">
      ...
      {selectedRunId && (
        <Sheet
          open={!!selectedRunId}
          onOpenChange={() => setSelectedRunId(null)}
        >
          <SheetContent
            style={{ minWidth: "80%" }}
      ...
      <EventDetails
        ...
        onOpenChange={isOpen => {
          if (!isOpen) setSelectedEventId(null);
        }}
      />
    </div>
  );
